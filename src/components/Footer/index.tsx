import React from "react";

export function Footer() {
    return (
      <footer className="mt-32 overflow-hidden">
        <div className="footer-grid">
          <div className="footer-info">
            <div>
              <h3 className="text-xl font-bold">Sobre o Site</h3>
            </div>
  
            <div>
              <p className="text-lg font-Rokkitt opacity-70 pt-2">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Impedit
                odit magni id veritatis natus fugiat facilis perferendis aliquid.
              </p>
            </div>
          </div>
  
          <div className="footer-info">
            <div>
              <h3 className="text-xl font-bold">Serviço de Atendimento</h3>
            </div>
            <ul>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Contato</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Reembolso/Troca</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Codigo de Desconto</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Lista de Desejos</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Produtos Especiais</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Mapa do Site</a>
              </p>
            </ul>
          </div>
  
          <div className="footer-info">
            <div>
              <h3 className="text-xl font-bold">Informações</h3>
            </div>
            <ul>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Sobre nos</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Entrega</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Política de Privacidade</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Suporte</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors">
                <a href="">Rastreamento de Pedido</a>
              </p>
            </ul>
          </div>
  
          <div className="footer-info">
            <div>
              <h3 className="text-xl font-bold">Criador</h3>
            </div>
  
            <ul>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">Alerrando</a>
              </p>
            </ul>
          </div>
  
          <div className="footer-info">
            <div>
              <h3 className="text-xl font-bold">Contatos</h3>
            </div>
            <ul>
              <p className="text-lg font-Rokkitt opacity-70 pt-2">
                291 South 21th Street, Suite 721 New York
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">(18) 1234-5678</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">info@seusite.com</a>
              </p>
              <p className="text-lg font-Rokkitt opacity-70 hover:opacity-100 transition-colors pt-2">
                <a href="">seusite.com</a>
              </p>
            </ul>
          </div>
        </div>
      </footer>
    );
  }
  